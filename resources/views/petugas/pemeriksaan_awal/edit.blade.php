        ...
        <form method="POST" action="{{ route('petugas.pemeriksaan_awal.update', $pasien->id) }}">
            ...
            <div class="mb-8 p-5 bg-slate-50 border border-slate-200 rounded-xl flex flex-col md:flex-row md:items-center gap-5">
                ...
                <div class="md:text-right">
                    ...
                    <p class="text-lg font-bold text-slate-600">#PX-{{ ($pasien->id ?? 0) + 1000 }}</p>
                </div>
            </div>
            ...
            <div class="mb-8">
                ...
                <textarea name="keluhan" id="keluhan" rows="4" placeholder="Tuliskan keluhan yang dirasakan pasien dengan detail..." required
                    class="w-full px-5 py-4 bg-white border border-slate-300 rounded-xl text-base font-medium text-slate-800 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all shadow-sm resize-y">{{ old('keluhan', $pasien->keluhan) }}</textarea>
                @error('keluhan')
                    <p class="text-rose-500 text-xs mt-1 font-semibold">{{ $message }}</p>
                @enderror
            </div>
