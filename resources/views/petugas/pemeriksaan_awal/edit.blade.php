                ...
                <div class="bg-slate-900 rounded-2xl p-8 text-white shadow-lg border border-slate-800 relative overflow-hidden">
                    ...
                    <div class="space-y-6 relative z-10">
                        <div>
                            ...
                            <p class="text-xl font-bold leading-tight uppercase tracking-tight">{{ $pasien->nama_pasien ?? '-' }}</p>
                        </div>
                        <div class="flex gap-4">
                            <div class="w-1.5 bg-emerald-500 rounded-full"></div>
                            <div>
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Unit Layanan / Poli Tujuan</p>
                                <p class="text-sm font-bold uppercase">{{ $pasien->poli ?? 'Umum' }}</p>
                            </div>
                        </div>
                        <div>
                ...
                <div class="bg-emerald-50 rounded-2xl p-6 border border-emerald-100">
                    ...
                    <ul class="text-sm text-emerald-700 space-y-3 font-medium">
                        <li class="flex items-start gap-2">
                            ...
                            Gunakan timbangan digital yang valid untuk Berat Badan (BB).
                        </li>
                        <li class="flex items-start gap-2">
                            <span class="font-black text-emerald-600">2.</span> 
                            Pastikan pasien berdiri tegak tanpa alas kaki saat mengukur Tinggi Badan (TB).
                        </li>
                        <li class="flex items-start gap-2">
                            <span class="font-black text-emerald-600">3.</span> 
                            Gunakan format tekanan darah standar sistolik/diastolik (misal: 120/80).
                        </li>
                    </ul>
                </div>
            </div>
                ...
                <div class="bg-white rounded-2xl p-8 lg:p-10 shadow-sm border border-slate-200">
                    <form method="POST" action="{{ route('petugas.pemeriksaan_awal.update', $pasien->id) }}" class="space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            {{-- BERAT BADAN --}}
                            <div>
                                <label for="berat_badan" class="flex items-center gap-2 text-sm font-bold text-slate-600 uppercase tracking-wide mb-2">
                                    ...
                                    Berat Badan (BB) <span class="text-rose-500">*</span>
                                </label>
                                <div class="relative">
                                    <input type="number" step="0.1" name="berat_badan" id="berat_badan" placeholder="0.0" required
                                        value="{{ old('berat_badan', $pasien->berat_badan) }}"
                                        class="w-full pl-5 pr-14 py-4 bg-slate-50 border border-slate-300 rounded-xl text-lg font-bold text-slate-800 focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all shadow-sm focus:bg-white placeholder:text-slate-300">
                            ...
                            {{-- TINGGI BADAN --}}
                            <div>
                                <label for="tinggi_badan" class="flex items-center gap-2 text-sm font-bold text-slate-600 uppercase tracking-wide mb-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" />
                                    </svg>
                                    Tinggi Badan (TB) <span class="text-rose-500">*</span>
                                </label>
                                <div class="relative">
                                    <input type="number" step="0.1" name="tinggi_badan" id="tinggi_badan" placeholder="0.0" required
                            ...
                            {{-- TENSI DARAH --}}
                            <div>
                                <label for="tensi" class="flex items-center gap-2 text-sm font-bold text-slate-600 uppercase tracking-wide mb-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                                    </svg>
                                    Tensi Darah <span class="text-rose-500">*</span>
                                </label>
                                <div class="relative">
                                    <input type="text" name="tensi" id="tensi" placeholder="120/80" required
                                        value="{{ old('tensi', $pasien->tensi) }}"
                                        class="w-full pl-5 pr-16 py-4 bg-slate-50 border border-slate-300 rounded-xl text-lg font-bold text-slate-800 focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all shadow-sm focus:bg-white placeholder:text-slate-300">
                        ...
                        {{-- KELUHAN --}}
                        <div>
                            <label for="keluhan" class="flex items-center gap-2 text-sm font-bold text-slate-600 uppercase tracking-wide mb-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                                Keluhan Utama <span class="text-rose-500">*</span>
                            </label>
                            <textarea name="keluhan" id="keluhan" rows="5" placeholder="Tuliskan keluhan yang dirasakan pasien dengan detail..." required
                                class="w-full px-5 py-4 bg-slate-50 border border-slate-300 rounded-xl text-base font-medium text-slate-800 focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all shadow-sm focus:bg-white resize-y shadow-sm">{{ old('keluhan', $pasien->keluhan) }}</textarea>
                            @error('keluhan')
                                <p class="text-rose-500 text-xs mt-1 font-semibold">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-100">
                            ...
                            <a href="{{ route('petugas.pemeriksaan_awal.index') }}" class="inline-flex items-center justify-center gap-2 bg-slate-100 text-slate-600 px-8 py-4 rounded-xl text-base font-bold hover:bg-slate-200 transition-colors border border-slate-300 sm:w-auto">
                                Batal
                            </a>
                        </div>
                    </form>
