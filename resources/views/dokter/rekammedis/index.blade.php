        ...
        <form method="GET" action="{{ route('dokter.rekammedis') }}" class="grid grid-cols-1 md:grid-cols-12 gap-5 items-end">
            ...
            <div class="md:col-span-2 flex gap-2">
                ...
                @if(request('q') || request('poli'))
                    <a href="{{ route('dokter.rekammedis') }}" class="px-4 py-3.5 bg-slate-100 text-slate-600 rounded-xl text-base font-bold hover:bg-slate-200 transition-colors border border-slate-300 flex items-center justify-center" title="Reset Filter">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <tr class="hover:bg-emerald-50/60 transition-colors group">
                        ...
                        <td class="py-5 px-6">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <a href="{{ route('dokter.rekammedis.show', $p->id) }}" class="font-extrabold text-slate-800 text-lg hover:text-emerald-600 transition-colors uppercase leading-tight block">
                                        {{ $p->nama_pasien ?? '-' }}
                                    </a>
                        ...
                        <td class="py-5 px-6 text-center align-middle">
                            <a href="{{ route('dokter.rekammedis.show', $p->id) }}" class="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-xl text-xs font-extrabold uppercase tracking-wider transition-colors shadow-sm cursor-pointer">
